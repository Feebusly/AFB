...
namespace Aga.Controls.Tree
{
	public class TreeModel : ITreeModel
	{
        ...
        private void GetNodes(Collection<Node> nodes, Node thisNode)
        {
            nodes.Add(thisNode);
            foreach (Node node in thisNode.Nodes)
            {
                GetNodes(nodes, node);
            }
        }
		...
		internal void OnNodeInserted(Node parent, int index, Node node)
		{
			...
			{
				TreeModelEventArgs args = new TreeModelEventArgs(
                    GetPath(parent), new int[] { index }, new object[] { node }, NodeChangeTypes.NodeInserted);
		...
		internal void OnNodeRemoved(Node parent, int index, Node node)
		{
			...
			{
				TreeModelEventArgs args = new TreeModelEventArgs(
                    GetPath(parent), new int[] { index }, new object[] { node }, NodeChangeTypes.NodeRemoved);
